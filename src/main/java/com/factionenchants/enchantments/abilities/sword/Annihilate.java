...
public class Annihilate extends CustomEnchantment {

    public Annihilate() {
        super("annihilate", "Annihilate", 6, EnchantTier.ULTIMATE, ApplicableGear.AXE);
    }
        ...
        if (target instanceof Player p) {
            for (ItemStack armor : p.getInventory().getArmorContents()) {
                if (armor != null && armor.getType() != org.bukkit.Material.AIR) {
                    ...
                    if (meta != null) {
                        int newDmg = meta.getDamage() + level * 10;
